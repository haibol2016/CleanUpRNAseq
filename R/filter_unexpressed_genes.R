...
#' the BAM files in a genome browser, like IGV first, to get a sense of the data.
#'
#' @param ensdb_sqlite A character(1), a path to an SQLite database for an
#'   [ensembldb::EnsDb-class] object.
...
#' @param min_samples An integer(1), minimal number of samples needed to consider
#'   a multi-exonic gene of `min_junction_counts` is expressed.
#'
#' @return A character vector of IDs of multi-exonic genes which are not
#'   supported by a minimal number of junction reads in minimal number of samples.
#' @export
...
get_unexpressed_spliced_genes <- function(ensdb_sqlite = NULL,
                                          ...
                                          min_samples = 1)
{
  if (length(ensdb_sqlite)!=1 || !file.exists(ensdb_sqlite))
  {
    stop("The SQLite file to a single EnsDb object DOES NOT exist!")
  }

  if (!is.data.frame(junction_counts) && !is.matrix(junction_counts)){
    stop("junction_count must be a data frame or a matrix!")
  }
  if (min_junction_counts < 1 || min_samples < 1)
  {
    stop("min_junction_counts and min_samples must be greater than or equal to 1.")
  }

  ensdb <- EnsDb(ensdb_sqlite)
  exon_GR <- exons(ensdb, columns = c("exon_id", "tx_id", "gene_id"))
  tx_exon_num <- as.data.frame(table(exon_GR$tx_id))
  colnames(tx_exon_num) <- c("tx_id", "exon_num")
  tx_exon_num <- merge(unique(data.frame(tx_id = exon_GR$tx_id,
                                         gene_id = exon_GR$gene_id)),
                       tx_exon_num, by = "tx_id")
  ...
  is_valid_junction <-
    apply(junction_counts[, 9:ncol(junction_counts)], 1,
          function(.x) {
...
#'   enough support of junction reads. An output of [get_unexpressed_spliced_genes()]
#'   function.
...
#'   *is_contaminated* column specifies whether the sample is DNA-contaminated,
#'   TRUE/FALSE, based on the various exploratory analyses using this package,
#'   such as read distribution by [check_read_distribution()], expression
#'   distribution by [check_expr_distribution()], percentage of expressed genes
#'   by [check_expressed_gene_percentage()], read distribution across different
#'   genomic features by [check_read_distribution()], and pairwise sample
#'   correlation by [check_sample_correlation()]. The *sample_name* and *group*
...
#' @param min_counts A numeric(1), minimal count cutoff threshold. Default is 10.
           ...
           min_tpm = 1)
{
  ...
  {
    stop("metadata should be a path to a file, a data frame, or a matrix.")
  }
  ...
  {
    stop("The column names of the raw count matrix DO NOT match the ",
         "sample names in the metadata!")
  }
  ## remove unexpressed multi-exonic genes
  if (!is.null(unexpressed_spliced_genes))
  {
    counts <- counts[!rownames(counts) %in% unexpressed_spliced_genes, ]
  }

  ## filter by TPM
  unexpressed_genes <-
    rownames(salmon_res$abundance)[rowSums(salmon_res$abundance >=
                                   min_tpm) == ncol(salmon_res$abundance)]
  counts <- counts[!rownames(counts) %in% unexpressed_genes, ]
  # check if uncontaminated samples exist in each distinct group. If so, filter
  # the count matrix by number of samples with expression above min_cpm
  ...
  uncontaminated_per_group <-
    length(unique(metadata$group[metadata$is_contaminated == FALSE])) ==
  ...
  if (uncontaminated_per_group) {
    ...
    uncontaminated_metadata <- metadata[uncontaminated_samples %in%
    ...
    uncontaminated_abundance <- salmon_res$abundance[, uncontaminated_samples]
    unexpressed_genes <-
      rownames(uncontaminated_abundance)[rowSums(uncontaminated_abundance >=
                                                   min_tpm) == ncol(uncontaminated_abundance)]
    counts <- counts[!rownames(counts) %in% unexpressed_genes, ]
  } else {
    # all samples are contaminated or samples in one group are all contaminated!

    # This operation might remove a small number of genes, which only express
    # the mono-exonic isoforms, though they can express multi-exonic isoforms
    # under other conditions (based on the GTF).

    keep <- rowSums(counts >= min_counts) >= min(table(metadata$group))
    counts <- counts[keep, ]

    # remove genes not expressed by min_counts and min_tpm in clean samples.
    # this operation (filter based on clean samples only from a single condition)
    # might remove some genes only expressed in contaminated samples (false
    # negatives).
