...
#'   with two columns: ucsc and ensembl for UCSC-style seqnames and Ensembl-style
#'   seqnames, respectively.
#'
                         ...
                         ensembl = c(1:22, "X", "Y", "MT")))
{
  ...
  {
    if (is.character(seqname_alias) && !file.exists(seqname_alias))
    {
      stop("seqname_alias is provided by a non-existing file!")
    }
    seqname_alias <- read.delim(seqname_alias, header = TRUE, as.is = TRUE)
  }
  ...
  if (any(!complete.cases(seqname_alias)) || any(duplicated(seqname_alias$ucsc)) ||
      any(duplicated(seqname_alias$ensembl)))
  {
      stop("missing or duplicated values are not allowed in seqname_alias!")
  }
  ...
  seqname_alias <- seqname_alias[seqname_alias$ucsc %in% sequences_to_keep,]
  UCSC_BSgenome <- keep_BSgenome_Sequences(UCSC_BSgenome, seqname_alias$ucsc)
...
#' writeLines(">chr1\nATCGCTGCGGATGCGG\n>chr2\nCCCCCCCCCCCGGGGATAAAA\n", in_fasta)
#' close(in_fasta)
#'
#' fa_dir <- generate_multifasta(genome_fasta = genome_fasta,
#'                               out_dir = out_dir,
#'                               prefix = "none")
#'
                                ...
                                prefix = c("none", "chr"))
{
  ...
  {
    stop("A path to a reference genome fasta file, genome_fasta,
  ...
  prefix <- match.arg(prefix, choices = c("none", "chr"), several.ok = FALSE)
  if (prefix == "none")
  {
     prefix  = ""
  }
  ...
  if (grepl(".(fa|fasta).gz$", genome_fasta))
  {
    ...
    stop("It seems the genome sequence file is not a valid fasta file ",
...
#'                               prefix = "none")
#'
#' seed_file <- generate_seed_file(multifasta_path = fa_dir,
#'                                 latin_name = "Homo sapiens",
#'                                 common_name = "Human",
#'                                 genome_version = "GRCh38",
#'                                 seed_file_name = file.path(out_dir,
#'                                                     "human.genome.seed.txt"),
#'                                 fasta_url = "http://ftp.ensembl.org/xxx.fa",
#'                                 release_date = "August 2007",
#'                                 source = "Ensembl",
#'                                 version = "1.0.0")
generate_seed_file <- function(multifasta_path = NULL,
                               latin_name = NULL,
                               common_name = NULL,
                               genome_version = NULL,
                               seed_file_name =NULL,
                               ...
                               version = "1.0.0")
{
  ...
  {
    stop("There are not multiple fasta files in the directory ",
         multifasta_path, "!")
  }
  ...
  cat(paste0("release_date: ", release_date, "\n"))
  cat(paste0("genome: ", genome_version, "\n"))
  ...
  chromosome_names <- gsub(".fa.gz$", "", dir(multifasta_path, "fa.gz$"))
  ...
  cat(paste0("seqs_srcdir: ", multifasta_path, "\n"))
...
#'                                 version = "1.0.0")
#'
#' forge_install_BSgenome(seed_file[2],
#'                        dest_dir = tempdir())
#'}
...
#' @param out_dir A character(1) specifying a path for output split, compressed
#'   fasta files
#' @param prefix A character(1), specifying chromosome prefix. It can be an empty
#'   string or "chr". Default is "".
#' @param latin_name A character(1), specifying the Latin name of the species of
#'   the reference genome, such as "Homo sapiens" for the human.
#' @param common_name A character(1), specifying the common name of the species
#'   of the reference genome, such as "Human".
...
#' close(in_fasta)
#'
#' make_BSgenome(genome_fasta = genome_fasta,
...
#'               version = "1.0.0")
#' }
make_BSgenome <- function(genome_fasta = NULL,
                          out_dir = tempdir(),
                          prefix = c("", "chr"),
                          latin_name = NULL,
                          common_name = NULL,
                          ...
                          version = "1.0.0")
{
  ...
  multifasta_path <- generate_multifasta(genome_fasta = genome_fasta,
                                         out_dir = out_dir,
                                         prefix = prefix)

  seed_file <- generate_seed_file(multifasta_path = multifasta_path,
  ...
  pkgname <- forge_install_BSgenome(seed_file = seed_file[2],
                                    dest_dir = out_dir)
  pkgname
}
