...
#' @param metadata A data frame or a path to a tab-delimited file, with column
#'   names: sample_name, group, BAM_file, and salmon_quant_file. The *sample_name*
#'   and *group* columns contain the unique sample labels and the experimental
#'   conditions for each sample, respectively. The *BAM_file* column contains
#'   the full paths to BAM files. The *salmon_quant_file* column contains the
#'   full paths to *quant.sf* files for quantitation of gene expression by Salmon
#'   pseudo-alignment. See <https://salmon.readthedocs.io/en/latest/>.The order
...
#'
#' @details A set of diagnostic plots will be generated and saved to the specified
                                      ...
                                      "TR_J_pseudogene","TR_V_pseudogene"))
{
    ...
    if (!is.data.frame(metadata) && !file.exists(metadata))
    {
      stop("metadata must be a tab-delimited file or a data frame.")
    } else if (!is.data.frame(metadata) && file.exists(metadata)) {
      metadata <- read.delim(metadata, header = TRUE, as.is = TRUE)
    } else if (any(!c("sample_name", "group", "batch", "BAM_file",
                      "salmon_quant_file") %in% colnames(metadata))) {
      stop("metadata must contain columns: sample_name, group, batch, BAM_file, ",
           "salmon_quant_file!")
    }
    ...
    {
      stop("Some BAM files, Salmon quant.sf files or sample names are ",
           "not unique.")
    }
    if(any(!file.exists(metadata$BAM_file)))
    {
      stop("some BAM files do not exist:\n",
    ...
    {
      stop("some Salmon quantification files do not exist:\n",
           paste(metadata$salmon_quant_file[!file.exists(metadata$salmon_quant_file)],
                 collapse = "\n"))
    }

    if (any(grepl("[^A-Za-z0-9_.]", metadata$group)||
            grepl("^_", metadata$group)))
    {
      stop("column 'group' in metadata contains special symbols, which ",
    ...
    if (length(out_dir) != 1 || !is.character(out_dir))
    {
      stop("out_dir is required to specify a single output directory!")
 ...
 # get SAF for genomic regions: genes, intron, exon, intergenic regions, rRNA,
 # organelle genome
 bamfile <- metadata$BAM_file[1]

 saf_list <- get_feature_saf(ensdb_sqlite = ensdb,
                             bamfile = bamfile,
                             mitochondrial_genome = mitochondrial_genome,
                             chloroplast_genome = chloroplast_genome)

 # summarize reads by featureCount
 count_res <- summarize_reads(metadata = metadata,
 ...
 salmon_summary <- salmon_res(metadata = metadata,
                              ensdb_sqlite = ensdb,
 ...
 pdf(file.path(out_dir, "Fig1.featureCount.read.assignment.statistics.pdf"),
     height = 3, width = 5)
 print(p1_featurecount_read_assignment)
 dev.off()

 # check read mapping rate across different genomic features: genes, introns,
 # exons, intergenic regions, rRNA, mitochondrial/plastid genome
 p2_mapping_rate_by_features_IR_rate <-
   check_read_distribution(featurecounts_list = count_res,
                           metadata = metadata)
 pdf(file.path(out_dir, "Fig2.read.mapping.rates.across.different.genomic.features.pdf"),
     height = 6, width = 8)
 print(p2_mapping_rate_by_features_IR_rate$p)
 dev.off()

 ## add IR_rate to metadata
 IR_rate <-  p2_mapping_rate_by_features_IR_rate$IR_rate
 metadata <- merge(metadata, IR_rate,
                   by.x = "sample_name",
                   by.y = "row.names",
                   all.x = TRUE,
                   sort = FALSE)
 ...
 tiff(filename = file.path(out_dir, "Fig3.sample.correlation.tiff"),
 ...
 p4_expression_distribution <-
   check_expr_distribution(counts = salmon_summary$counts,
                           normalization = normalization,
                           metadata = metadata)

 pdf(file.path(out_dir, "Fig4.expression.distribution.pdf"),
     height = 9, width = 6)
 print(wrap_plots(p4_expression_distribution,
                  nrow = 3, ncol = 1))
 dev.off()
 ...
 p5_percent_expressed_genes <-
   check_expressed_gene_percentage(metadata = metadata,
            counts = salmon_summary$counts,
            min_cpm = 1,
            abundance = salmon_summary$abundance,
            min_tpm = 1)
 pdf(file.path(out_dir, "Fig5.percentages.of.genes.expressed.pdf"),
     height = 6, width = 8)
 print(p5_percent_expressed_genes)
 dev.off()

 # check sample similarity and variation by hierarchical clustering and PCA
 p6_pca_heatmap <- exploratory_analysis(counts = salmon_summary$counts,
                                        metadata = metadata)

 pdf(file.path(out_dir, "Fig6.heatmap.showing.sample.similarity.pdf"),
     height = 6, width = 8)
 print(p6_pca_heatmap$heatmap)
 dev.off()
 pdf(file.path(out_dir, "Fig7.PCA.showing.sample.variability.pdf"),
     height = 6, width = 8)
 print(p6_pca_heatmap$pca)
 dev.off()

 diagnosis_res <-
   list(p1_featurecount_read_assignment = p1_featurecount_read_assignment,
        p2_mapping_rate_by_features = p2_mapping_rate_by_features_IR_rate$p,
  ...
  saveRDS(diagnosis_res, file = file.path(out_dir, "Diagnostic.plots.objects.RDS"))
  saveRDS(metadata, file = file.path(out_dir, "metadata.with.IR.rates.RDS"))
 diagnosis_res
}
