...
#' saf_list <- get_feature_saf(ensdb_sqlite = EnsDb.Hsapiens.v86,
#'                             bamfile = bam_file,
...
{
  if (length(bamfile) != 1 || !file.exists(bamfile)) {
  ...
  gene_GR <- genes(ensdb, filter = SeqNameFilter(rownames(genome_info))) %>%
    reduce_ranges()
  gene_saf <- granges_to_saf(gene_GR)

  exon_GR <- exons(ensdb, filter = SeqNameFilter(rownames(genome_info))) %>%
    reduce_ranges()
  exon_saf <- granges_to_saf(exon_GR)

  ##chromosome/scaffolds GRanges

  genome_GR <- data.frame(seqnames = rownames(genome_info),
                          start = 1L,
...
#'   paired-end or not.
#' @param strandSpecific An integer(1), specifying the strandedness of the RNA-seq
...
#'   strand).See <https://salmon.readthedocs.io/en/latest/library_type.html> and
...
#'   such as the output of the [get_feature_saf()] function.
#' @param gtf A character(1), specifying a path to a GTF file.
#' @param threads An integer(1), number of threads for [Rsubread::featureCounts()]
#'   calling.
...
#' saf_url <- "https://drive.google.com/uc?export=download&id=1jbYiGY44Js2Y58A7nDl85dwRf4Hq0MHm"
#' download.file(url = saf_url,
#'               destfile = file.path(tmp_dir, "hs_saf_list.RData"),
#'               mode = "wb")
#' load(file.path(tmp_dir, "hs_saf_list.RData"))
#'
#' counts_list <- summarize_reads(metadata = metadata,
#'                                isPaired = TRUE,
#'                                strandSpecific = 0,
#'                                saf_list = saf_list,
                            ...
                            verbose = FALSE)
{
  ...
  {
    stop("Not all column names: sample_name, BAM_file, group are included ",
         "in the metadata.")
  }
  ...
  if (length(saf_list) < 5 || any(!c("gene", "exon", "intergenic_region",
                                     "intronic_region", "rRNA") %in%
                                   names(saf_list)))
  {
     stop("A valid SAF list for gene, exon, intergenic region,",
  ...
  if (length(strandSpecific) != 1 || !strandSpecific %in% c(0:2))
  {
    stop("strandSpecific must be 0, 1, or 2.")
  }

  saf_list$gtf <- gtf
  count_res <- mapply(function(annotation, isGTF, junction){
    res <- featureCounts(files = bamfiles,
                  # annotation
    ...
    {
      colnames(res$counts_junction)[9:ncol(res$counts_junction)] <- sample_name
    }
...
#'   `stat` element in the sublist `gtf` in the output of the [summarize_reads()]
#'   function. You can access it via `featureCounts()$gtf$stat`.
#'
...
#'
#' p <- check_read_assignment_stat(assignment_stat = counts_summary$gtf$stat)
#' print(p)
#' }
...
check_read_assignment_stat <- function(assignment_stat = NULL)
{
  ...
  if (!is.data.frame(assignment_stat) || (is.data.frame(assignment_stat) &&
      colnames(assignment_stat)[1] != "Status"))
  {
      stop("assignment_stat must be a data frame with column",
      " 1 named as 'Status'")
  }

  assignment_stat <- assignment_stat[rowSums(assignment_stat[, -1]) != 0, ]
  ...
  assignment_count_long <- melt(assignment_stat,
                          id.vars= "Status",
                          variable.name="Sample",
                          value.name="Statistics")
  assignment_count_long$Stat <- "Count"
  assignment_pct_long <- melt(assignment_stat_pct,
                                id.vars= "Status",
                                variable.name="Sample",
                                value.name="Statistics")
  assignment_pct_long$Stat <- "Percentage"
  assignment_long <- rbind(assignment_count_long, assignment_pct_long)

  assignment_long$Sample <- factor(assignment_long$Sample,
                                   levels = unique(assignment_long$Sample))

  p <- ggplot(assignment_long, aes(x = Sample, y = Statistics,
                                   fill = Status)) +
...
#'   \describe{
#'   \item{p}{A ggplot object, showing percentages of fragments assigned to different
#'   genomic features, such as genic regions, intergenic regions, exonic regions,
#'   intronic regions, rRNA genes, mitochondrial genome, chloroplast genome
#'   (only for plants)}
...
#' p <- check_read_distribution(featurecounts_list = counts_summary,
#'                              metadata = metadata)
#' print(p$p)
#' }
check_read_distribution <-
  function(featurecounts_list = NULL,
           metadata = data.frame(sample_name = vector(mode = "character"),
                                 group = vector(mode = "character")))
{
  if (!is.list(featurecounts_list) || any(!c("counts", "annotation", "stat") %in%
      names(featurecounts_list[[1]])))
  {
    stop("featurecounts_list must be a list of lists, each sublist containing ",
         "featureCounts output for different genomic features, such as output ",
  ...
  if (!is.data.frame(metadata))
  {
    ...
    stop("metadata is a data frame, but it does not contain columns: sample_name ",
         "or group.")
  }


  assigned_per_region <-mapply(function(.x, .y){
    assigned <- .x$stat[.x$stat$Status == "Assigned", , drop = FALSE]
    ...
    assigned
  }, featurecounts_list[-length(featurecounts_list)],  ## no GTF-base summary
     names(featurecounts_list)[-length(featurecounts_list)],
  SIMPLIFY = FALSE)

  assigned_per_region <- do.call("rbind", assigned_per_region)
  total_mapped_frags <- colSums(featurecounts_list$gtf$stat[, -1, drop = FALSE])
  total_mapped_frags <- rep(total_mapped_frags, length(featurecounts_list) -1)
  assigned_per_region$assigned_percent <-
                       assigned_per_region$assigned_count / total_mapped_frags * 100

  levels = c("gene", "exon", "intronic_region",
               "intergenic_region", "rRNA",
               "mitochondrion")
  labels = c("Gene", "Exon", "Intron",
               "Intergenic region", "rRNA",
               "Mitochondrion")
  ...
  {
    stop("sample names in metadata are not consistent with those in ",
         "featurecounts_list.")
  }
  ...
  assigned_per_region <-
    assigned_per_region[order(assigned_per_region$group,
  ...
  p <- ggplot(assigned_per_region,
              aes(x = sample_name,
                  y = assigned_percent,
                  color = group)) +
    geom_point() + xlab("Sample") +
    ylab("Assigned reads (%)") +
    ...
    theme(text = element_text(size = 8),
          axis.text.x = element_text(angle = 90,
                                     ...
                                     hjust=0,
                                     size = 8))
  IR_rate <- assigned_per_region[assigned_per_region$region_type ==
                                   "Intergenic region", ]
...
#'
#' Generate a panel of plots based on a count table, with the diagonal showing the
#' sample names, the lower triangle showing smoothed scatterplots for gene
#' expression of pairwise samples, and the upper triangle showing Pearson
#' correlation coefficients of gene expression of pairwise samples.
#'
#' @param counts A numeric matrix or data frame containing gene expression count
#'   data from an RNA-seq experiment, with row names for gene ID and column names
...
#'   use the count table outputted by the [summarize_reads()] function with a GTF
#'   file as annotation.
                       ...
                       group = vector(mode = "character")))
{
    ...
    {
      stop("Column names of the raw count matrix DO NOT match ",
           "sample names in the metadata!")
    }
  metadata <- metadata[order(metadata$group, metadata$sample_name), ]
  ...
  if (normalization == "qsmooth")
  {
    ...
    counts_qs <- qsmooth(object = counts,
  ...
  counts_1_log_long <- melt(counts_1_log,
                            id.vars= "GeneID",
                            variable.name="Sample",
                            value.name="log2Count")
  counts_1_log_long <- merge(counts_1_log_long,
                             metadata[, c("sample_name", "group")],
                             by.x = "Sample",
                             by.y = "sample_name",
                             all.x = TRUE)
  counts_1_log_long <- counts_1_log_long[order(counts_1_log_long$group,
  ...
  p_box <- ggplot(counts_1_log_long,
                  aes(x = Sample,
                      y = log2Count,
                      fill = group)) +
    geom_boxplot()+
  ...
  {
    p_density <- ggplot(counts_1_log_long,
                        aes(x = log2Count,
                            group = Sample,
                            color = group))+
      guides(color = guide_legend(title="Group"))
  ...
  cpms_log_long <- melt(cpms_log,
                        id.vars= "GeneID",
                        variable.name="Sample",
                        value.name="log2CPM")
  cpms_log_long <- merge(cpms_log_long,
                         metadata[, c("sample_name", "group")],
                         by.x = "Sample",
                         by.y = "sample_name",
                         all.x = TRUE)
  cpms_log_long <- cpms_log_long[order(cpms_log_long$group,
...
#' @return A ggplot object if `counts` is not `NULL`, showing percentages of genes
#'   with counts above the user-specified minimal CPM (count per million) in
#'   each sample. Or a ggplot object of two panels if both `counts` and `abundance`
#'   are not `NULL`, showing percentages of genes  with counts above the
#'   user-specified minimal CPM and minimal TPM (transcript per million) in each
#'   sample.
#' @details
#'  The axis title contains unicode, so please output the plot in the svg format
#'  using the svglite package, instead of [grDevices::pdf()], for high-resolution
#'    plot.
#'
#'
#' @export
...
#' load(file.path(tmp_dir, "read_count_summary.RData"))
#'
#' p <- check_expressed_gene_percentage(metadata = counts_summary$metadata,
           ...
           min_tpm =1)
{
    if (!is.data.frame(metadata))
    {
      ...
      stop("Not all column names: sample_name and group are included ",
           "in the metadata.")
    }

    if (!is.null(counts) && !is.matrix(counts) && !is.data.frame(counts))
    {
      stop("A count table must be provied as a matrix or",
    ...
    {
      stop("An abundance table must be provied as a",
    ...
    if (!is.numeric(min_cpm) || length(min_cpm) != 1 || min_cpm <= 0)
    {
      stop("min_cpm must be a single positive number")
    }
    if (!is.numeric(min_tpm) || length(min_tpm) != 1 || min_tpm <= 0)
    {
  ...
  # counts <- counts[rowSums(counts)> 0, ]
  if ((!is.null(counts) && any(!colnames(counts) %in% metadata$sample_name)) ||
      (!is.null(abundance) && any(!colnames(abundance) %in% metadata$sample_name)))
  {
  ...
  if (!is.null(counts))
  {
    ...
    pct_cpm_gt1 <- sapply(cpms,
                          function(.x){sum(.x >= min_cpm)}) / nrow(counts) * 100

    pct_cpm_gt1_df <- data.frame(sample_name = names(pct_cpm_gt1),
                                 percent = pct_cpm_gt1)
    pct_cpm_gt1_df <- merge(pct_cpm_gt1_df, metadata,
                            by = "sample_name",
                            all.x = TRUE)
    pct_cpm_gt1_df <- pct_cpm_gt1_df[order(pct_cpm_gt1_df$group,
  ...
  if (!is.null(abundance))
  {
    ...
    pct_tpm_gt1 <- sapply(abundance,
                          function(.x){sum(.x >=
                                             min_tpm)}) / nrow(abundance) * 100

    pct_tpm_gt1_df <- data.frame(sample_name = names(pct_tpm_gt1),
                                 percent = pct_tpm_gt1)

    pct_tpm_gt1_df <- merge(pct_tpm_gt1_df, metadata,
                            by = "sample_name",
                            all.x = TRUE)
    pct_tpm_gt1_df <- pct_tpm_gt1_df[order(pct_tpm_gt1_df$group,
  ...
  if (!is.null(counts) && !is.null(abundance))
  {
    ...
    p <- ggplot(cpm_tpm, aes(x = sample_name,
                             y = percent,
                             color = group)) +
      geom_point() + xlab("Sample") +
    ...
    p <- ggplot(pct_cpm_gt1_df, aes(x = sample_name,
                                    y = percent,
                                    color = group)) +
      geom_point() + xlab("Sample") +
    ...
    p <- ggplot(pct_tpm_gt1_df, aes(x = sample_name,
                                    y = percent,
                                    color = group)) +
      geom_point() + xlab("Sample") +
...
#'                c(1, 3, 2, 4)), IRanges(1:10, width=10:1))
...
granges_to_saf <- function(granges)
{
  ...
  saf <- as.data.frame(granges)[, c(1:3, 5)]
