...
#'
#' @param region A data frame containing the columns: "Chr",	"Start",
#'   "End", and "Strand", such as a data frame contained in a sublist named
#'   *intergenic_region* from the output of the [get_feature_saf()] function, or
#'   a object of [GenomicRanges::GRangesList-class] or
...
#' ucsc_seqnames <-
#'      seqnames(ucsc_BSgenome)[!grepl("_alt|fix|hap\\d+", seqnames(ucsc_BSgenome))]
#'
#' ## Wierd: BSgenome.Hsapiens.UCSC.hg19 has both chrM and chrMT for mitochondrial
...
#'
#' seqname_alias <- data.frame(ucsc = ucsc_seqnames, ensembl = ensembl_seqnames)
#' bsgenome <- UCSC2Ensembl(UCSC_BSgenome = BSgenome.Hsapiens.UCSC.hg38,
#'                          genome_version = "GRCh38",
#'                          seqname_alias = seqname_alias)
#' region <- data.frame(GeneID = as.character(1:10),
#'                     Chr = rep("1", 10),
#'                     Start = 100000* (1:10),
#'                     End = 100000* (1:10) + 1000,
#'                     Strand = rep("+", 10))
#'
#' gc_contents <-  calculate_region_gc(region = region,
#'                                     BSgenome = bsgenome,
#'                                     batch_size = 2000)
                                ...
                                batch_size = 2000)
{
  ...
  if (!is.null(region))
  {
     if (is.data.frame(region) &&
         any(c("GeneID", "Chr",	"Start", "End",
               "Strand") != colnames(region)))
     {
       stop("region is a data farme, but not in a valid format. ",
            'It must contain the following columns: "GeneID", "Chr",	"Start"',
            '"End", and "Strand".')
     } else if (!is.data.frame(region) && !is(region, "GRanges") &&
                !is(region, "GRangesList")) {
       stop("region is not a SAF, thus it must be providedas a object of ",
  ...
  if (is.data.frame(region))
  {
    ...
    {
      stop("Some chromosome names in the intergenic SAF data frame is not ",
           "in the BSgenome object.")
    }
    grouping <- rep(1:ceiling(nrow(region) / batch_size),
                    each = batch_size)[1:nrow(region)]
    ...
    grouping_f <- rep(1:ceiling(length(region) / batch_size),
                      each = batch_size)[1:length(region)]
    grouping <- split(1:length(region), grouping_f)
    if (is(region, "GRangesList"))
    {
      gc_contents <- do.call(rbind, lapply(grouping, function(.x)
        {
          sequences <- getSeq(BSgenome,
                              region[.x],
                              as.character=FALSE)
          gc_contents <- do.call(rbind, lapply(sequences, function(.y) {
            gene_size <- sum(width(.y))
            gene_gc <-
              sum(letterFrequency(.y, "GC", as.prob = FALSE)[, 'G|C']) / gene_size
      ...
      gc_contents <- do.call(rbind, lapply(grouping, function(.g){
        gc_contents <- getSeq(BSgenome, region[.g], as.character=FALSE) %>%
          letterFrequency("GC", as.prob = TRUE) %>%
          as.data.frame()
        colnames(gc_contents) <- "gc_content"
...
#'                          genome_version = "GRCh38",
#'                          seqname_alias = seqname_alias)
#' gene_contents <- calculate_gene_gc(ensdb_sqlite = edb,
#'                                    BSgenome = bsgenome,
#'                                    batch_size = 2000)
#'}
calculate_gene_gc <- function(ensdb_sqlite = NULL,
                              ...
                              batch_size = 2000)
{
  if (length(ensdb_sqlite)!=1 || (!is(ensdb_sqlite, "EnsDb") &&
                                  !file.exists(ensdb_sqlite)))
  {
    stop("The ensdb_sqlite must be an EnsDb object or a SQLite file ",
         "to an EnsDb object!")
  }

  if (!is(ensdb_sqlite, "EnsDb") && file.exists(ensdb_sqlite))
  {
  ...
  exons_grlist <- exonsBy(ensdb, by = "gene",
                          filter = SeqNameFilter(value = common_seqnames),
                          use.names = FALSE)
  exons_grlist <- reduce(exons_grlist)
  gene_gc <- calculate_region_gc(region = exons_grlist,
                                 BSgenome = BSgenome,
                                 ...
                                 plot = TRUE)
{
  estimate_contamination <- function(.count, .sample_name, .gc)
  {
    ...
    keep <- #count_per_base >= quantile(count_per_base, probs = 0.01) &
    ...
    l <- loess(count_per_base ~ .gc$gc_content,
               family="symmetric", span = 0.25)

    data <- cbind(.gc, count_per_base,
    ...
    temp_data <- split(temp_data, f = cut(temp_data$gc_content,
                                          breaks = seq(0, 1, 0.05)))
    perc5_bin_cpb <- sapply(temp_data,
                            function(.x){quantile(.x$CPB, prob = 0.5)})
    perc5_bin_cpb[10:20] <- c(perc5_bin_cpb[9:1], 0, 0)
    list(contamination = perc5_bin_cpb, plot_data = data)
  }

  contamination <- mapply(function(.intergenic_count, .sample_name,
                          .intergenic_gc)
  {
    contamination_level <- estimate_contamination(.intergenic_count,
                                                  .sample_name,
                                                  .gc = .intergenic_gc)
  }, intergenic_counts, colnames(intergenic_counts),
  ...
  gc_bin_contamination <- do.call(cbind, lapply(contamination, "[[", 1))
  rownames(gc_bin_contamination) <- gsub("\\.\\d+%", "",
...
#'
#' Correct DNA contamination considering GC-bias effect on fragment amplification.
#' Intergenic regions are binned based on their GC content ranging from 0 to 100%,
#' with a bin size of 5%. Per gene DNA contamination is estimated as the product
#' of count per base in a GC content matching bin of intergenic regions and the
#' total collapsed exons of a gene and is subtracted away from the gene count
#' matrix.
#'
#'
...
#'
#'gene_gc_content_url <-
#'  paste0("https://www.dropbox.com/scl/fi/taoj18w5t4up22d04xihk/GRCh38.gene.",
...
#'   paste0("https://www.dropbox.com/scl/fi/ik5z5etbr63nnv43jna8c/GRCh38.",
#'         "intergenic.GC.content.RDS?rlkey=kxrn6ixdnvhpba9h7w2r265xp&dl=1")
#' intergenic_gc_destfile <- file.path(tmp_dir,
#'                                    "GRCh38.intergenic.GC.content.RDS")
#' download.file(url= intergenic_gc_content_url,
...
#'   gc_bias_correction(salmon_res = salmon_quant,
#'                     gene_gc = gene_gc,
#'                     intergenic_counts =
                               ...
                               plot = FALSE)
{
  ...
  if (!is.data.frame(intergenic_counts) && !is.matrix(intergenic_counts))
  {
  ...
  {
    stop("colnames of the Salmon count table is not exactly the same as those ",
    "of the intergenic_counts table")
  }

  if (!all(rownames(salmon_res$counts) %in% rownames(gene_gc)) ||
      !all(rownames(gene_gc) %in% rownames(salmon_res$counts)))
  {
    warning("Gene names in the Salmon count table is not exactly the same as\n",
    "those of the gene_gc data frame. The intersection of gene names from both\n",
    "sources will be used.")

    common_genes <- intersect(rownames(salmon_res$counts), rownames(gene_gc))
    if (length(common_genes) < 1000)
    {
      stop("There are only", length(common_genes), "genes shared between the ",
      "Salmon count table and the gene_gc data frame. Please double check the ",
      "rownames.")
    }
    salmon_res <- lapply(salmon_res[1:3], function(.x){
      .x <- .x[common_genes, ]
    })
  ...
  {
    stop("rownames of intergenic_gc DO NOT match those of intergenic_counts!")
  }
  ...
  gene_contamination <- gene_gc_sample[, -c(1,2)] * salmon_res$length
