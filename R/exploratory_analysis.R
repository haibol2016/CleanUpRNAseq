                                 ...
                                 group = vector(mode = "character")))
{
  ...
  if (!all(colnames(counts) == metadata$sample_name))
  {
    stop("colnames of the count table don't match the sample names in the ",
         "metadata.")
  }
  ...
  p_pheatmap <- distancePlot(sampleDists = sampleDists,
                             sampleNames = vsd$sample_name,
                             metadata = metadata)

  ## PCA plot showing PC1 and PC2 only
  pca <- prcomp(t(assay(vsd)), scale = TRUE, center = TRUE, retx = TRUE)
